...
class SelfHealingMesh:
    ...
    def __init__(self, consensus_threshold: int = 2, healing_period_seconds: int = 3600):
    ...
    def register_node(self, node: Node, scope: CapabilityScope | None = None) -> None:
    ...
    def complete_healing(self, node_id: str) -> HealingResult:
        ...
        if not self.state_machine.can_promote_to_active(node, self.healing_period_seconds):
            return HealingResult(
                node_id=node_id,
                success=False,
        ...
        try:
            self.state_machine.transition(node, NodeState.ACTIVE, reason="Healing complete")
            result = HealingResult(
                node_id=node_id,
                success=True,
    ...
    def reroute(
        ...
        if exclude_path:
            return self.router.find_alternate_path(
                source_id, target_id, data_categories, self._nodes, exclude_path
            )
        return self.router.find_path(source_id, target_id, data_categories, self._nodes)
    ...
    def get_detector(self, node_id: str) -> AttackDetector | None:
        """Get the detector for a node."""
        return self._detectors.get(node_id)

    def get_healing_results(self, node_id: str | None = None) -> list[HealingResult]:
